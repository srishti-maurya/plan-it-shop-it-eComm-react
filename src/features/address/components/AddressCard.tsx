...
export function AddressCard({ address, onEdit }: AddressCardProps) {
  ...
  return (
    <div className="relative rounded-lg border bg-white p-4 shadow-sm transition-shadow hover:shadow-card">
      {address.isDefault && (
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        >
          <FaEdit className="mr-1 inline h-3 w-3" />
          Edit
        </Button>
        <Button
          variant="ghost"
        ...
        >
          <FaTrash className="mr-1 inline h-3 w-3" />
          Delete
        </Button>
        {!address.isDefault && (
          <Button
